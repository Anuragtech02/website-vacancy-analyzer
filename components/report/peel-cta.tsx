...
export function PeelCTA({ onUnlock, currentScore }: PeelCTAProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full bg-black rounded-3xl overflow-hidden shadow-xl group cursor-pointer flex items-center p-3 sm:p-5 gap-2 sm:gap-4"
      onClick={onUnlock}
    >
        ...
        <div className="relative w-[120px] sm:w-[220px] shrink-0 h-full max-h-[300px] flex items-center">
           ...
           <div className="relative w-full aspect-[3/4] sm:aspect-[4/5] rounded-2xl shadow-2xl transition-transform duration-500 hover:scale-[1.02]">
               ...
               <motion.div
                 ...
                 initial={false}
                 animate={{
                     width: isVisible ? [initialFold, teaseFold, initialFold] : initialFold,
                     height: isVisible ? [initialFold, teaseFold, initialFold] : initialFold,
                 }}
                 style={{
